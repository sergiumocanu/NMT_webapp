...
univariate <- function(args){
  ...
  HTMAP_KEY_XLAB <- str_remove_all(args[16], "[\"]")
  HTMAP_KEY_YLAB <- str_remove_all(args[17], "[\"]")
  HTMAP_MARGIN <- str_remove_all(args[18], "[\"]")
  HTMAP_MARGIN <- eval(parse(text = HTMAP_MARGIN))
  ...
  PCA_PC <- str_remove_all(args[23], "[\"]")
  PCA_PC <- eval(parse(text = PCA_PC))
  PCA_BIPLOT_SAMPLELABEL_TYPE <- str_remove_all(args[24], "[\"]")
  ...
  VOLCANO_SIG_COLOUR <- str_remove_all(args[44], "[\"]")
  VOLCANO_NONSIG_COLOUR <- str_remove_all(args[45], "[\"]")
  ...
  SIG_HTMAP_KEY_XLAB <- str_remove_all(args[54], "[\"]")
  SIG_HTMAP_KEY_YLAB <- str_remove_all(args[55], "[\"]")
  SIG_HTMAP_MARGIN <- str_remove_all(args[56], "[\"]")
  SIG_HTMAP_MARGIN <- eval(parse(text = SIG_HTMAP_MARGIN))
  SIG_HTMAP_WIDTH <- as.numeric(args[57])
  SIG_HTMAP_HEIGHT <- as.numeric(args[58])
  SIG_PCA_PC <- str_remove_all(args[59], "[\"]")
  SIG_PCA_PC <- eval(parse(text = SIG_PCA_PC))
