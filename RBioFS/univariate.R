...
univariate <- function(args) {
  ###### sys variables --------
  # ------ file name variables ------
  DAT_FILE <- args[6]  # 2D file
  MAT_FILE_NO_EXT <-
    args[7]  # from the raw mat file, for naming export data
  NODE_FILE <- args[8]
  NODE_ID_VAR <- args[61]
  ...
  y <-
    factor(raw_sample_dfm$group, levels = unique(raw_sample_dfm$group))
  ...
  pair <-
    data.frame(ProbeName = seq(ncol(raw_sample_dfm) - 2),
               pair = colnames(raw_sample_dfm)[-c(1:2)])
  sample <-
    paste0(raw_sample_dfm$sampleid, "_", raw_sample_dfm$group)
  ...
  connections <-
    foreach(i = as.character(rawlist$genes$pair), .combine = "c") %do% {
      # import node info
      ids <- unlist(strsplit(i, "_"))
      region1 <-
        node[, REGION_NAME_VAR][node[, NODE_ID_VAR] %in% ids[1]]
      region2 <-
        node[, REGION_NAME_VAR][node[, NODE_ID_VAR] %in% ids[2]]
  ...
  if (HTMAP_LAB_ROW) {
    rbioarray_hcluster(
      ...
      ctrlProbe = FALSE,
      rmControl = FALSE,
      srtCol = HTMAP_TEXTANGLE_COL,
      offsetCol = 0,
      key.title = "",
      dataProbeVar = "pair",
      cexCol = HTMAP_TEXTSIZE_COL,
      cexRow = HTMAP_TEXTSIZE_ROW,
      keysize = HTMAP_KEYSIZE,
      key.xlab = HTMAP_KEY_XLAB,
      key.ylab = HTMAP_KEY_YLAB,
      plotWidth = HTMAP_WIDTH,
      plotHeight = HTMAP_HEIGHT,
      margin = HTMAP_MARGIN
    )
  } else {
    rbioarray_hcluster(
  ...
  pca_all <-
    data.frame(
      normdata$targets[, c("group", "sample")],
      t(normdata$E),
      stringsAsFactors = FALSE,
      check.names = FALSE,
      row.names = NULL
  ...
  rbioFS_PCA(
    ...
    sampleIDVar = "sample",
    groupIDVar = "group",
    scaleData = PCA_SCALE_DATA,
    centerData = PCA_CENTRE_DATA,
    boxplot = TRUE,
    boxplot.Title = NULL,
    boxplot.Width = PCA_WIDTH,
    boxplot.Height = PCA_HEIGHT,
    biplot = TRUE,
    biplot.comps = PCA_PC,
    biplot.Title = NULL,
    ...
    biplot.SymbolSize = PCA_BIPLOT_SYMBOL_SIZE,
    biplot.ellipse = PCA_BIPLOT_ELLIPSE,
    biplot.ellipse_conf = PCA_BIPLOT_ELLIPSE_CONF,
    biplot.xAngle = 0,
    biplot.xhAlign = 0.5,
    biplot.xvAlign = 0.5,
    biplot.loadingplot = FALSE,
    biplot.loadingplot.textsize = PCA_BIPLOT_LOADING_TEXTSIZE,
    biplot.mtx.densityplot = PCA_BIPLOT_MULTI_DESITY,
    biplot.mtx.stripLblSize = PCA_BIPLOT_MULTI_STRIPLABEL_SIZE,
    biplot.Width = PCA_WIDTH,
    biplot.Height = PCA_HEIGHT,
    rightsideY = PCA_RIGHTSIDE_Y,
    fontType = "sans",
    xTickLblSize = PCA_X_TICK_LABEL_SIZE,
    yTickLblSize = PCA_Y_TICK_LABEL_SIZE,
    verbose = FALSE
  )
  ...
  contra_string <-
    gsub(" ", "", contra_string, fixed = TRUE)  # remove all the white space
  # NOTE: below: use do.call to unpack arguments for makeContrasts()
  contra <-
    do.call(makeContrasts, c(as.list(contra_string), levels = as.list(parse(text = "design"))))
  ...
  tryCatch(
    rbioarray_DE(
      ...
      genesymbolVar = "pair",
      topgeneLabel = TRUE,
      nGeneSymbol = VOLCANO_N_TOP_CONNECTION,
      padding = 0.5,
      FC = UNI_FOLD_CHANGE,
      ctrlProbe = FALSE,
      ...
      symbolSize = VOLCANO_SYMBOL_SIZE,
      sigColour = VOLCANO_SIG_COLOUR,
      nonsigColour = VOLCANO_NONSIG_COLOUR,
      xTxtSize = VOLCANO_X_TEXT_SIZE,
      yTxtSize = VOLCANO_Y_TEXT_SIZE,
      plotWidth = VOLCANO_WIDTH,
      plotHeight = VOLCANO_HEIGHT,
  ...
  DE_summary  <-
    as.data.frame(get(paste0(MAT_FILE_NO_EXT, "_DE_summary")))
  names(DE_summary) <-
    c("comparison",
      "p-value.threshold",
  ...
  for (i in 1:length(get(paste0(MAT_FILE_NO_EXT, "_DE")))) {
    if (DE_summary[i, "sig"] == 0) {
      ...
      super_cluster_data <-
        list(
          E = normdata$E[, de_sample_idx],
          genes = normdata$genes,
          targets = normdata$targets[de_sample_idx, ]
        )
      rbioarray_hcluster_super(
        plotName = paste0(MAT_FILE_NO_EXT, "_DE_", de_names[i]),
        fltDOI = super_cluster_data,
        dfmDE = get(paste0(MAT_FILE_NO_EXT, "_DE"))[[i]],
        DE.sig.method = sig.method,
        FC = UNI_FOLD_CHANGE,
        DE.sig.p = UNI_ALPHA,
        clust = "complete",
        ctrlProbe = FALSE,
        ...
        dataProbeVar = "pair",
        rowLabel = TRUE,
        annot = super_cluster_data$genes,
        annotProbeVar = "pair",
        genesymbolVar = "pair",
        ...
        adjCol = c(1, 0),
        key.title = "",
        keysize = SIG_HTMAP_KEYSIZE,
        scale = c("row"),
        cexCol = SIG_HTMAP_TEXTSIZE_COL,
        cexRow = SIG_HTMAP_TEXTSIZE_ROW,
        srtCol = SIG_HTMAP_TEXTANGLE_COL,
        key.xlab = SIG_HTMAP_KEY_XLAB,
        key.ylab = SIG_HTMAP_KEY_YLAB,
        margin = SIG_HTMAP_MARGIN,
        plotWidth = SIG_HTMAP_WIDTH,
        plotHeight = SIG_HTMAP_HEIGHT,
        verbose = FALSE
      )
    }
  }
  ...
  if (length(contra_string) == 1) {
    if (UNI_FDR) {
      if (FDR_FAIL_WARNING) {
        ...
        pcutoff <-
          max(fit_dfm$P.Value[which(fit_dfm$adj.P.Val <= UNI_ALPHA)])
      }
    } else {
      pcutoff <- UNI_ALPHA
    }
    sig_pairs_fit <-
      as.character(fit_dfm[fit_dfm$P.Value < pcutoff, "pair"])
  } else {
    if (UNI_FDR) {
      pcutoff <-
        tryCatch(
  ...
  if (length(sig_pairs_fit) <= 1) {
    ...
    pca_sig <-
      pca_all[, names(pca_all) %in% c("group", "sample", sig_pairs_fit), drop = FALSE]
    rbioFS_PCA(
      input = pca_sig,
      sampleIDVar = "sample",
      groupIDVar = "group",
      ...
      biplot.SymbolSize = PCA_BIPLOT_SYMBOL_SIZE,
      biplot.ellipse = PCA_BIPLOT_ELLIPSE,
      biplot.ellipse_conf = SIG_PCA_BIPLOT_ELLIPSE_CONF,
      biplot.xAngle = 0,
      biplot.xhAlign = 0.5,
      biplot.xvAlign = 0.5,
      biplot.loadingplot = PCA_BIPLOT_LOADING,
      biplot.loadingplot.textsize = PCA_BIPLOT_LOADING_TEXTSIZE,
      biplot.mtx.densityplot = PCA_BIPLOT_MULTI_DESITY,
      biplot.mtx.stripLblSize = PCA_BIPLOT_MULTI_STRIPLABEL_SIZE,
      biplot.Width = PCA_WIDTH,
  ...
  ml_dfm <-
    data.frame(sampleid,
               ...
               stringsAsFactors = FALSE)
  write.csv(
    file = paste0(RES_OUT_DIR, "/", MAT_FILE_NO_EXT, "_ml.csv"),
    ml_dfm,
    row.names = FALSE
  )
  ...
  cat("-------------------------------------\n")
  cat(
    "Hierarchical clustering heatmap saved to: \n",
    paste0(MAT_FILE_NO_EXT, "_hclust_all.pdf\n")
  )
  cat("\n")
  ...
  if (UNI_FDR &&
      FDR_FAIL_WARNING)
    cat("No significant results found with FDR, using raw p value instead.\n")
  print(DE_summary)
  cat("\n")
  ...
  cat("\t",
      paste0(MAT_FILE_NO_EXT, "_", contra_string, "_DE.csv"),
      "\n")
  cat("\n")
  cat(
    "Volcano plot saved to file(s): \n",
  ...
  if (length(contra_string) == 1) {
    ...
    if (NO_SIG_WARNING_FIT) {
      cat(
        "NO significant reuslts found in F-stats results, no PCA needed. Program terminated. \n"
      )
    }
  }
}
